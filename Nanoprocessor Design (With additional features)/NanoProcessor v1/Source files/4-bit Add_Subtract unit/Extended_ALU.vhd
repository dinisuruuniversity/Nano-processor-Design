...
entity Extended_ALU is
    Port (
        ...
        Op       : in  STD_LOGIC_VECTOR(2 downto 0); -- Operation select (= opcode)
...
begin
    ...
    -- -------------------------------------------------------------------------
    process(A, B, Op, result_int, product_int)
    begin
        result_int  <= (others => '0');
        ...
        case Op is
            ...
            when "010" =>  -- NEG: 0 - B  (two's complement of B)
                result_int <= std_logic_vector(
                    ('0' & unsigned("0000")) - ('0' & unsigned(B)));

            when "011" =>  -- MUL: A x B (full 8-bit product)
                product_int <= std_logic_vector(unsigned(A) * unsigned(B));
                result_int  <= '0' & product_int(3 downto 0);

            when "100" =>  -- AND: bitwise A AND B
                result_int <= '0' & (A and B);

            when "101" =>  -- OR: bitwise A OR B
                result_int <= '0' & (A or B);
    ...
    Overflow <= result_int(4) when (Op = "000" or Op = "001" or Op = "010") else
                '1'           when (Op = "011" and product_int(7 downto 4) /= "0000") else
                '0';

...
entity NanoProcessor is
    Port (
        ...
        LED   : out STD_LOGIC_VECTOR(15 downto 0);  -- [3:0]=R1, [4]=Zero, [5]=Overflow
...
begin
    ...
    Slow_Clock_Inst : Slow_Clk
        port map ( Clk_in => Clk, Clk_out => slow_clk_sig,Reset   => Reset );
    ...
    --     lSlt='1' ?? ALU instruction: write ALU result (registerValue input)
    --     lSlt='0' ?? MOVI: write immediate value     (immediateValue input)
    --  (Decoder sets Load_Select='1' for all ALU ops, '0' for MOVI)
    -----------------------------------------------------------------------
    ...
    -----------------------------------------------------------------------
    LUT_Inst : LUT_16_7
        port map ( address => reg_outputs(7),
                   data    => seg );
    ...
    -----------------------------------------------------------------------
    -- 13. 4-bit Comparator
    --     Compares the values of Operand-A and Operand-B
    -----------------------------------------------------------------------
    Comparator_Inst : Comparator_4_bit
        port map (
            Input_A           => reg_a_data,
            Input_B           => reg_b_data,
    ...
    -- 14. LED outputs
    -----------------------------------------------------------------------
    LED(3 downto 0) <= reg_outputs(7);  -- R7 result

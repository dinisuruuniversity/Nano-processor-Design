...
-- Description:  Top-level entity that wires all sub-components of the 4-bit
--               NanoProcessor together.
--
--  Instruction Set:
--    10 = MOVI  Rd, imm4   → Rd ← imm4
--    00 = ADD   Rd, Rs     → Rd ← Rd + Rs
--    01 = NEG   Rd         → Rd ← 0  - Rd  (negate via R0-Rd)
--    11 = JZR   Rs, addr3  → if Rs=0 then PC ← addr3
--
--  Board : Basys 3 (Xilinx Artix-7)
--    Clk   → W5  (100 MHz oscillator)
--    Reset → U18 (btnC, active-high)
--    LED(3:0) → display R1 result on rightmost 4 LEDs
--    seg(6:0) → 7-segment cathodes (result on rightmost digit)
--    an(3:0)  → digit anodes (only an(0) active → rightmost digit)
--
-- Compile / source order for Vivado:
--   1.  Packages/BusDefinitions.vhd
--   2.  Packages/constants.vhd
--   3.  3-bit Adder/HA.vhd
--   4.  3-bit Adder/FA.vhd
--   5.  3-bit Adder/RCA_3.vhd
--   6.  4-bit Add_Subtract unit/RCA_4.vhd
--   7.  4-bit Add_Subtract unit/Add_Sub_4_bit.vhd
--   8.  Decoders/Decoder_2_to_4.vhd
--   9.  Decoders/Decoder_3_to_8.vhd
--   10. D Flip Flop/D_FF.vhd
--   11. Register Bank/Reg.vhd
--   12. Register Bank/Register_Bank.vhd   (use Packages/BusDefinitions, NOT the local copy)
--   13. MUX/MUX_2W_3B.vhd
--   14. MUX/MUX_2W_4B.vhd
--   15. MUX/MUX_8W_4B.vhd
--   16. Program Counter/Program_Counter.vhd
--   17. Program Counter Adder/Program_Counter_Adder.vhd
--   18. Adress_Selector/Address_selector.vhd
--   19. Load_Selector/Load_Selector.vhd
--   20. Instruction Decoder/Instruction_Decoder.vhd
--   21. Look up tables/LUT_16_7.vhd
--   22. Slow_Clock/Slow_Clk.vhd
--   23. Program Rom/Program_ROM.vhd
--   24. NanoProcessor.vhd            ← this file (top)
----------------------------------------------------------------------------------
...
begin
    ...
    -----------------------------------------------------------------------
    -- 13. LED output  (R1 value → 4 rightmost LEDs)
    -----------------------------------------------------------------------
    LED <= reg_outputs(1);
